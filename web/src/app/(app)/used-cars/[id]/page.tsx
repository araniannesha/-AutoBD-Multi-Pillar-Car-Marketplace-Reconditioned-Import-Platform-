...
export default async function UsedCarDetailPage({
  ...
  let offers: { id: string; amount: string; buyerName: string; status: OfferStatus; createdAt: Date }[] = [];
                      ...
                      {o.status === OfferStatus.PENDING && !isSold ? (
                        ...
                        <div>
                          ...
                          <div className="text-xs text-muted">
                            {new Date().getTime() - new Date(o.createdAt).getTime() > 7 * 24 * 60 * 60 * 1000 ? (
                              <span className="font-bold text-red-600">Expired</span>
                            ) : (
                              <span>Expires in {Math.ceil((7 * 24 * 60 * 60 * 1000 - (new Date().getTime() - new Date(o.createdAt).getTime())) / (24 * 60 * 60 * 1000))} days</span>
                            )}
                          </div>
                        </div>
                      )}
